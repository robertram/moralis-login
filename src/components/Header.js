    ...
    <div className="">
      <nav className="flex items-center justify-between flex-wrap bg-teal-500 p-6">
        <div className="flex items-center flex-shrink-0 text-white mr-6">
          ...
          <div className="flex flex-col md:flex-row hidden md:block -mx-2 ml-4">
            <a
              href="/signUp"
      ...
      <div>{isAuthenticating ? "loading" : ""}</div>
      {authError && <div className="text-red">{authError.message}</div>}
    </div>
  );
};
